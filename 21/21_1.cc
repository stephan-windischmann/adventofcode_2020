...
int solve(const std::map<std::string, std::vector<std::string> > &input,
    std::map<std::string, int> &ingrCount) {
    int res = 0;
    ...
    for (auto const &ingr : ingrCount) {
        if (found.find(ingr.first) == found.end()) {
            res += ingr.second;
        }
    }
